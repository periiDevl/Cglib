...
static void drawLine(float x1, float y1, float z1, float x2, float y2, float z2, float r, float g, float b, float width);
static void drawCone(float x, float y, float z, float axisX, float axisY, float axisZ, float r, float g, float b, float size);
static void drawCircle(float x, float y, float z, float radius, int axis, float r, float g, float b, float width);
static void drawCube(float x, float y, float z, float size, float r, float g, float b);
static void worldToScreen(float wx, float wy, float wz, int sw, int sh, float* sx, float* sy, float* depth);
static bool isNearPoint(float mx, float my, float px, float py, float threshold);
static void getCameraVectors(float* rightX, float* rightY, float* rightZ, float* upX, float* upY, float* upZ);
...
void GIZMO_init(Gizmo* gizmo, CubeObject* targetObject) {
    ...
    gizmo->targetObject = targetObject;
    gizmo->lastMouseX = gizmo->lastMouseY = 0;
    gizmo->dragPlaneX = gizmo->dragPlaneY = gizmo->dragPlaneZ = 0;
}
